...
def hasRequired(raw: String) =
  required.forall(raw.contains(_))

def isValid(raw: String) =
  raw
    .replace("\n", " ")
    .split("\\s")
    .forall(isValidPart)

def isValidPart(p: String): Boolean = p.trim match
  case s"byr:$year" => intBetween(1920, 2002, year.toInt)
  case s"iyr:$year" => intBetween(2010, 2020, year.toInt)
  case s"eyr:$year" => intBetween(2020, 2030, year.toInt)
  case s"hgt:$height" => isValidHeight(height)
  case s"hcl:#$color" => isValidColor(color)
  case s"ecl:$color" => validColors.contains(color.toLowerCase)
  case s"pid:$id" => id.length == 9
  case s"cid:$id" => true
  case _ => false

def isValidHeight(h: String) = h match
  case s"${value}cm" => intBetween(150, 193, value.toInt)
  case s"${value}in" => intBetween(59, 76, value.toInt)
  case _ => false

def isValidColor(c: String) = (c.length == 6) && (Integer.parseInt(c, 16) <= 16777215)

def intBetween(start: Int, end: Int, value: Int) =
