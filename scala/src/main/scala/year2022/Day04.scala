...
def countFullyContained(ranges: (Range, Range)): Boolean =
  ranges._1.intersect(ranges._2).size == Math.min(ranges._1.size, ranges._2.size)

def countOverlap(ranges: (Range, Range)): Boolean =
  ranges._1.intersect(ranges._2).nonEmpty
